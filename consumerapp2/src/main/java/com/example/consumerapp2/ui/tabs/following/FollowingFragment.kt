...
import com.example.consumerapp2.databinding.FragmentFollowingBinding
...
class FollowingFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (favorites != null) {
            dataFavorite =
                (activity!!.intent.getParcelableExtra(DetailActivity.EXTRA_FAVORITE) as Favorite?)!!
            getUserFollowing(dataFavorite.username)
        } else {
            dataUser =
                (activity!!.intent.getParcelableExtra(FollowersFragment.EXTRA_DATA) as User?)!!
