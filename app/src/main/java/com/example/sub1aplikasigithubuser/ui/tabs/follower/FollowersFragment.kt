package com.example.sub1aplikasigithubuser.ui.tabs.follower
...
import com.example.sub1aplikasigithubuser.ui.detail.DetailActivity.Companion.EXTRA_FAVORITE
...
class FollowersFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        favorites = requireActivity().intent.getParcelableExtra(EXTRA_FAVORITE)
        if (favorites != null) {
            dataFavorite =
                (requireActivity().intent.getParcelableExtra(EXTRA_FAVORITE) as Favorite?)!!
            getUserFollowers(dataFavorite.username)
        } else {
            dataUser = (requireActivity().intent.getParcelableExtra(EXTRA_DATA) as User?)!!
